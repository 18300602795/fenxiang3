...
//        int onlyWifi = tasksManagerModel.getOnlyWifi();//1
        ...
        switch (status) {
            ...
            case ApkDownloadStatus.RETRY://重试
                apklDownloadListener.prepareDown(tasksManagerModel, !wifiConnected && onlyWifi == 1, true);
                break;
        }
    }

    public static void onClick(String gameId, ApklDownloadListener apklDownloadListener, boolean isError) {
        if (gameId == null) return;
        //使用设置中的开关全局控制是否非wifi下载
//        int onlyWifi = tasksManagerModel.getOnlyWifi();//1
        int onlyWifi = SP.getBoolean(AileConstants.SP_4G_DOWN, false) ? 0 : 1;
        ...
        if (tasksManagerModel == null) {
            ...
            if (!BaseAppUtil.isOnline(BaseApplication.getInstance())) {
                ...
                apklDownloadListener.prepareDown(tasksManagerModel, !wifiConnected && onlyWifi == 1, isError);
            }
            return;
        }
        L.i("333", "已经存在task");
...
//                apklDownloadListener.prepareDown(tasksManagerModel, !wifiConnected && onlyWifi == 1);
    ...
    private static BaseDownloadTask createDownTask(TasksManagerModel baseGameDownModel) {
        L.i("333", "url：" + baseGameDownModel.getUrl());
        ...
        baseGameDownModel.setPath(FileDownloadUtils.getDefaultSaveFilePath(baseGameDownModel.getUrl()));
        TasksManager.getImpl().updateTask(baseGameDownModel);
